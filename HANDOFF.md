...
- УТП серий — РЕАЛИЗОВАНО. Команды `utp-collect` (сбор кандидатов с сайтов
  ...
  MDV берётся НЕ с сайта, а из официального прайс-листа: `utp-mdv <прайс.xlsx>`
  (модуль `mdv_pricelist.py`) — там УТП по всем сериям, включая on/off; имена серий
  прайса сопоставляются с JAC по вхождению токенов. Команда заменяет только строки MDV,
  сохраняя строки и галочки остальных брендов.
